...
trait ProposalValidation
{
    ...
    public function validateProposalIsEditable($data, $type = 'proposal', $abort = null)
    {
        if (!in_array($data->status, ['Draft', 'Rejected'])) {

            $message = 'Tidak bisa merubah data ' . $type . ' karena sudah diajukan';
            if ($abort) {
                abort(403, $message);
            } else {
    ...
    public function validateProposalOwnership($data, $type = 'proposal', $return = 'Exception')
    {
        if ((Auth::user()->userable_type == UnitKemahasiswaan::class && $data->unit_id == Auth::user()->userable_id) || Gate::allows('admin')) {
            return;
        } else {
            $message = 'Anda tidak memiliki akses ke ' . $type . ' ini';
            if ($return == 'Exception') {
                throw new \Exception($message);
            } else {
                abort(403, $message);
